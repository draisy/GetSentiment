  ...
  def inject_get_method(klass)
    ...
    klass.class_eval do
      ...
      def get(annotation, anno_base = nil)
        ...
        #my addition
        if StanfordCoreNLP.camel_case(annotation) == 'ClassName' || StanfordCoreNLP.camel_case(annotation) == 'AnnotatedTree'
          anno_class = "#{StanfordCoreNLP.camel_case(annotation)}"
        elsif StanfordCoreNLP.camel_case(annotation) == 'PredictedClass'
          anno_class = "#{StanfordCoreNLP.camel_case(annotation)}"      
        else
